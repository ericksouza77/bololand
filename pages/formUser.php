...
if(!empty($_POST)){
    ...
    $sqlUser = "insert into usuario (nome, email, tel, numero, complemento, cep) value ('$nome', '$email', '$tel', '$numero', $complemento, '$senha', '$cep'";
    
    $conn = mysql_connect("localhost", "root","","bololand");
    mysql_set_connect($conn, "utf8");
    mysql_query($conn, htmlspecialchars($sql)) or die (mysql_error($conn));
    mysql_close($conn);
}
?>
